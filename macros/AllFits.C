...
//	wspace.factory(("Nele["+std::to_string(SigEle[type])+","+std::to_string(SigEle[type]-sqrt(SigEle[type]))+","+std::to_string(SigEle[type]+sqrt(SigEle[type]))+"]").c_str());
	...
	std::string variables = "B_mass,hnl_mass,hnl_charge,hnl_lxy,LxyBin,dr_trgmu_hnl,dilepton_mass,hnl_vtxProb,hnl_l_pt,hnl_l_mvaId,hnl_pi_pt,hnl_l_dxyS,hnl_pi_dxyS,dilepton_pt,likelihood,hnl_lxy_sig,LepQProd";
...
//	wspace.var("hnl_mass")->setMax(3.3);
	RooDataSet mcSet("mcSet","mcSet",*wspace.set("treeSet"),RooFit::Import(*c),RooFit::Cut((sel).c_str()));
...
//	RooFitResult* res = signal->fitTo(mcSet,RooFit::Range(mcSet.mean(*wspace.var("hnl_mass"))-1*mcSet.sigma(*wspace.var("hnl_mass")),mcSet.mean(*wspace.var("hnl_mass"))+1*mcSet.sigma(*wspace.var("hnl_mass"))),RooFit::Save());
	...
	signal.plotOn(mhnl_frame,RooFit::Name("signal"),RooFit::Range(mass-0.3,mass+0.2),RooFit::LineColor(2),RooFit::MoveToBack()); // this will show fit overlay on canvas
...
/*	TPaveText* t = new TPaveText(0.1,0.8,0.3,0.62,"NB");
	...
	mhnl_frame->addObject(latex);
  	latex->DrawLatex(mass-0.28,mhnl_frame->GetMaximum()*0.92,("#chi^{2}/n.dof"+std::string(Form(" %.2f", chi2))).c_str());
  	latex->DrawLatex(mass-0.28,mhnl_frame->GetMaximum()*0.97,("lxy bin "+std::to_string(lxyBin)+", HNL Mass"+std::string(Form(" %.2f", mass))+" GeV c#tau = "+std::string(Form(" %.2f", ctau))+ " mm").c_str());
	...
	RooAbsReal*  yield = signal.createIntegral(*wspace.var("hnl_mass"),RooFit::Range("twoSigma"));
