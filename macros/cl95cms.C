...
static Double_t A0, sA, B0, sB, epsilon, MaxSig = 10000.;
...
void UL_1fb(Double_t s, Double_t b, Int_t mass);
...
Double_t CL95(Double_t ilum, Double_t slum, Double_t eff, Double_t seff, Double_t bck, Double_t sbck, Int_t n, Bool_t gauss, Int_t nuisanceModel)
{
	...
	B0 = bck;
	sB = sbck;
	epsilon = 1.e-4;//TMath::Max(Precision/ilum,1.e-4);
	//
	lGauss = gauss;
	...
	/*
	if (plot) cout << "likelihodd = " << Likelihood(x,p) << endl; 
	while (delta > 0)
	{
	...
	if (sB == 0.)
	{
		...
		else 
		{
			...
			if (I == 0)
			{
				...
				while (tmp > epsilon)
				{	 
					tmp = in->Integral(low,high,epsilon);
	...
	// This function can make use of function Inner, which also takes into account uncertainty in lumi*effi.
{
	...
	else 
	{
		...
		Double_t low = A0 > 5.*sA ? A0 - 5.*sA : 0.;
		if (I == 0){
		std::cout << " low " <<low << "upper lim" << A0+5.*sA<< "Gaus mean" << B0<< "gaus sigma" << sB<<  std::endl;
		retval = TMath::Gaus(x[0],B0,sB,kTRUE)*in->Integral(low,A0+5.*sA);
		std::cout << " here in Outer " <<retval<< std::endl;
		}
		...
		else if (I == 1) retval = TMath::LogNormal(x[0], sigma_b, 0., B0)*in->Integral(0.,A0+5.*sA,epsilon);
//		else if (I == 2) retval = TMath::GammaDist(x[0], sigma_b, 0., 1./tau_b)*in->Integral(0.,A0+5.*sA,par,epsilon);
		else if (I == 2) retval = TMath::GammaDist(x[0], sigma_b, 0., 1./tau_b)*in->Integral(0.,A0+5.*sA,epsilon);
		else return 0;
		delete in;
	}
	return retval;
...
void UL_1fb(Double_t s, Double_t b, Int_t mass) {
	...
	float nTotalEvents;
	if( mass==200 ) nTotalEvents = 109110.;
	else if( mass==250 ) nTotalEvents = 109981.;
	else if( mass==300 ) nTotalEvents = 109101.;
	else if( mass==350 ) nTotalEvents = 106452.;
	else if( mass==400 ) nTotalEvents = 109924.;
	else if( mass==450 ) nTotalEvents = 109906.;
	else if( mass==500 ) nTotalEvents = 102857.;
	else {
		std::cout << "mass " << mass << " currently not supported." << std::endl;
		exit(11818);
	}

	float crossSection;
	if( mass==200 )      crossSection = (5.4218+0.64243+0.015972+0.056746+0.10830)*0.2537*0.067316*0.7*2.;
	else if( mass==250 ) crossSection = (3.4120+0.43490+0.0073067+0.023553+0.044954)*0.2951*0.067316*0.7*2.;
	else if( mass==300 ) crossSection = (2.5001+0.30317+0.0038098+0.010992+0.021079)*0.3053*0.067316*0.7*2.;
	else if( mass==350 ) crossSection = (2.4983+0.21635+0.0021911+0.0055876+0.010794)*0.3023*0.067316*0.7*2.;
	else if( mass==400 ) crossSection = (2.0608+0.15739+0.0054837+0.00030308+0.0059045)*0.2664*0.067316*0.7*2.;
	else if( mass==450 ) crossSection = (1.3445+0.11630+0.00087374+0.0017291+0.00033991)*0.2582*0.067316*0.7*2.;
	else if( mass==500 ) crossSection = (0.83396+0.087068+0.00058401+0.0010272+0.0020377)*0.2602*0.067316*0.7*2.;
	else {
		std::cout << "mass " << mass << " currently not supported." << std::endl;
		exit(11818);
	}

	crossSection *= 1000.; //in fb

	float eventWeight = crossSection / nTotalEvents;

	float nEvents_passed = s/eventWeight;

	float effS = nEvents_passed / nTotalEvents;

	float ul = 1000.*CLA( 1000., 0., effS, 0., b, 0. );

	std::cout << std::endl << std::endl;
	std::cout << "  ------> UL (1 fb-1, no errors) : " << ul<< " fb" << std::endl;
	std::cout << std::endl << std::endl;
